...
function crearCards() {
   juegos.forEach(juego=>{
      ...
      <p>$${juego.valor}</p> 
      <p>%${juego.descuento}</p> 
      <button class="btnCarrito" id="btn-agregar${juego.id}">Agregar al changuito</button>
      </div>`
   }) 
   funcionBoton();  
}
...
function agregarAlCarrito(el){
   ...
   if(existe===false){
      ...
      let juegoFind = carrito.find(juegoSome=> juegoSome.id === el.id)
      juegoFind.cantidad++
   }
   console.log(existe);
 ...
 function renderizarCarrito(){
    ...
    carrito.forEach(juego=>{
      ...
      <p> ${juego.nombre} x ${juego.cantidad}</p>

      <p>Valor real $${(juego.valor * juego.cantidad)*(((100 - juego.descuento)/100)*1.75)}</p>  

      <button id="btn-borrar${juego.id}">Eliminar</button>
